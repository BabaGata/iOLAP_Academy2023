...
GLOBAL_TN = 'avujic-academy-global'
JOBS_TN = 'avujic-academy-jobs'
...
def getting_global_uri():
    ...
    URI = global_config['Item']['url']['S'].format(method=METHOD, host=HOST, port=PORT, prefix=PREFIX)
    PARTITIONS_URI = global_config['Item']['partitions_uri']['S']
    return URI, PARTITIONS_URI
...
def save_partitons(partitions, min_ingestion_dttm, job_table_uri, URI, table_name):
    for partition in partitions.json():
        if check_latest_date(min_ingestion_dttm, dt.datetime.strptime(partition, "%Y%m%dT%H%M%S.%f")):
            partition_r = requests.get(URI + job_table_uri + M_I_D + partition)
...
def lambda_handler(event, context):
    ...
    URI, PARTITIONS_URI = getting_global_uri()
    min_ingestion_dttm, job_table_uri = getting_table_cofiguration(table_name)
    
    partitions = requests.get(URI + PARTITIONS_URI.format(table_name=table_name))
    ...
    if check_latest_date(min_ingestion_dttm, latest_partition):
        save_partitons(partitions, min_ingestion_dttm, job_table_uri, URI, table_name)
